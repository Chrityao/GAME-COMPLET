function doublepend2
% Double pendulum animation
% The differential equations are solved using Euler's method.
% Taken form http://www.myphysicslab.com/dbl_pendulum.html

% Constants 
m1 = 3; %mass of 1st bob     %red
m2 = 5; %mass of 2nd bob     %yellow
...
while (t <= 50) && ishandle(h)
    ...
    omega1n = omega1+dt*(-g*(2*m1 + m2)*sin(theta1)...
        -m2*g*sin(theta1-2*theta2)-2*sin(theta1-theta2)...
        *m2*(omega2^2*l2+omega1^2*l1*cos(theta1-theta2)))...
        /(l1*(2* m1 + m2 - m2*cos(2*theta1 - 2*theta2)));

    omega2n = omega2+dt*(2*sin(theta1-theta2)*(omega1^2*l1*(m1 + m2)...
